...
class SharedTokenRefresherTest {
    ...
    @Test
    fun `successful refresh writes the new pair atomically and returns it`() =
        runTest {
            val storage = InMemoryTokenStorage()
            val refresher =
                SharedTokenRefresher(
                    storage = storage,
    ...
    @Test
    fun `hard failure clears stored tokens and returns null`() =
        runTest {
            val storage = InMemoryTokenStorage()
    ...
    @Test
    fun `soft failure preserves stored tokens and returns null for caller retry`() =
        runTest {
            val storage = InMemoryTokenStorage()
    ...
    @Test
    fun `caller carrying a stale access token sees the already-refreshed pair without re-issuing`() =
        runTest {
            ...
            val storage =
                InMemoryTokenStorage(initialAccess = "freshly-refreshed", initialRefresh = "freshly-refreshed-refresh")
    ...
    @Test
    fun `concurrent callers all see exactly one refresh operation fire`() =
        runTest {
            ...
            results.forEach { pair ->
                assertEquals(
                    BearerPair("new-access", "new-refresh"),
                    pair,
                    "all callers must see the same refreshed pair",
                )
            }
        }
}
