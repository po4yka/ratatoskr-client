...
class LanguagePreferenceRepositoryImplTest {
    private fun newRepo(seed: Map<String, Any> = emptyMap()): Pair<LanguagePreferenceRepositoryImpl, ObservableSettings> {
    ...
    @Test
    fun `default on first launch is System when nothing has been written`() = runTest {
        val (repo, _) = newRepo()
        assertEquals(LanguagePreference.System, repo.getLanguagePreference().first())
    }

    @Test
    fun `write then read round-trips the preference`() = runTest {
        val (repo, _) = newRepo()
    ...
    @Test
    fun `picking System clears the override and the flow reports it`() = runTest {
        val (repo, _) = newRepo()
        repo.updateLanguagePreference(LanguagePreference.Russian)
    ...
    @Test
    fun `unknown stored key collapses to System rather than throwing`() = runTest {
    ...
    @Test
    fun `storage key is the bare enum name so dumps stay human-readable`() = runTest {
